...
class ExecutorTasks extends \Democontent2\Pi\User implements IApi
{
    private $errorCode = 1;
    ...
    public function run(HttpRequest $request)
    {
        if ($this->checkKey($request)) {
            if ($this->getId()) {
                ...
                if (count($userParams)) {
                    $items = new Items();
                    $items->setUserId($this->getId());

                    if ($request->get('status')) {
                        $items->setStatus(intval($request->get('status')));
                    }

                    $result = $items->getWhereExecutor();
                    ...
                    if (count($result)) {
                        ...
                        foreach ($result as $item) {
                            ...
                            $this->setId($item['UF_OWNER_ID']);
                            $userParams = $this->get();

                            $reviews->setUserId($item['UF_OWNER_ID']);
                            $rating = $reviews->rating();
