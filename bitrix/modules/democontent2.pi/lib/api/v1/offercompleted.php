...
class OfferCompleted extends \Democontent2\Pi\User implements IApi
{
    private $errorCode = 1;
                ...
                if ($request->get('taskId')) {
                    if ($request->get('categoryId')) {
                        ...
                        if ($ownerId > 0 && $ownerId !== $this->getId()) {
                            ...
                            $response->setIBlockId($item->getIBlockId());
                            $executor = $response->checkExecutor();
                            if ($executor['userId'] > 0 && $executor['userId'] == $this->getId()) {
                                $item->executorCompleted($ownerId);
                                $this->errorCode = 0;
                            } else {
                                $this->errorMessage = 'You`re not is executor of this task';
                            }
                        } else {
