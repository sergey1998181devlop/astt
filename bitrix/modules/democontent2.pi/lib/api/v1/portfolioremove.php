...
class PortfolioRemove extends \Democontent2\Pi\User implements IApi
{
    private $errorCode = 1;
    ...
    public function run(HttpRequest $request)
    {
        if ($request->get('id') && intval($request->get('id'))) {
            if ($this->checkKey($request)) {
                ...
                if ($portfolioCategory->remove(intval($request->get('id')))) {
                    $this->errorCode = 0;
                } else {
                    $this->errorMessage = 'Failed to delete portfolio section';
                }
