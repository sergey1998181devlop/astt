...
//        debmes($this->arResult);
        $this->arResult['c'] = $item->getDemoContent( $this->arResult['UF_ID']);
//        debmes($this->arResult['c']);
        if($USER->IsAuthorized()){
            $favourites = new \Democontent2\Pi\FavouritesTask(intval($USER->GetID()));
            $this->arResult['status_executor'] = $favourites->getElement($USER->GetID() , $this->arResult['UF_ID'] );
        }
...
//                            $photo = $reviews->getListByTask();
                    ...
                    if ($request->isPost()) {
                        if ($request->getPostList()->get('moderation')) {
                            switch ($request->getPostList()->get('moderation')) {
                                case 'reject':
                                    $item->refusal_app( $this->arResult['UF_ID'] , $this->arResult['ID_STATUS_TASK']);
                                    break;
                                case 'approve':
                                    $item->approve();
                                    break;
