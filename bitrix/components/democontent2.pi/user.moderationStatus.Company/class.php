...
class UserSettings extends CBitrixComponent
{
    public function executeComponent()
    {
        ...
        if ($USER->IsAuthorized()) {
            ...
            $this->arResult['COMPANY'] =  $us->getCompanyManager($dir[4]);
//            debmes( $this->arResult['COMPANY'] );

            if($this->arResult['COMPANY'][0]['UF_STATUS_MODERATION'] !== "1"){
                LocalRedirect( "/user/moderation/" , true);
            }
...
//            die();
            ...
            if ($request->isPost()) {
                ...
                if ($request->getPost('type')) {
                    ...
                    if ($request->getPost('addCard')) {
                        ...
                        if ($request->getPost('falseModeration')) {

                            $us->falseModeration();


                            if (strlen($us->getRedirect())) {
                                LocalRedirect('/user/moderataion/', true);
                            }
                        }
                        if ($request->getPost('TrueModeration')) {
                            

                            $us->TrueModeration($this->arResult['COMPANY']);
                        ...
                        if ($request->getPost('setExecutor')) {
                            ...
                            if ($request->getPostList()->get('specialization') && $request->getPostList()->get('executorDescription')) {
                                $data = [
            ...
            $APPLICATION->AddChainItem('Модерация' , '/user/moderation/');
            $APPLICATION->AddChainItem($this->arResult['COMPANY'][0]['UF_COMPANY_NAME_MIN']);
