...
class DetailComponentClass extends CBitrixComponent
{
    ...
    public function executeComponent()
    {
        ...
        if($USER->IsAuthorized()){
            ...
            $_POST['CODETABLE'] = 'Democontentpiall';
        ...
        if ($request->isPost()) {
            ...
            if ($request->getPostList()->get('moderation')) {
                ...
                switch ($request->getPostList()->get('moderation')) {
                    case 'reject':
                        $this->arResult['taskId'] = $this->arResult['UF_ITEM_ID'];
                        $this->arResult['iBlockId'] = $this->arResult['UF_IBLOCK_ID'];


                        $item->refusal_app_moderation($this->arResult['UF_ITEM_ID']  , $this->arResult['UF_IBLOCK_ID']  , $this->arResult['c']['ID'] , $this->arResult['UF_USER_ID']);
                        break;
                    case 'approve':
          
                        $item->approveModeration($this->arResult['UF_ITEM_ID']  , $this->arResult['c']['ID']  , $this->arResult['UF_IBLOCK_ID'] , $this->arResult['UF_USER_ID']);
                        break;
...
//        debmes($this->arResult);
        if (count($this->arResult) > 0 && isset($this->arResult['UF_ITEM_ID']) && intval($this->arResult['UF_ITEM_ID']) > 0) {
...
//                            $photo = $reviews->getListByTask();
            ...
            if (!intval($this->arResult['UF_MODERATION']) || $USER->IsAdmin()) {
                ...
                */
                ...
                $IBLOCK_ID = '';
                $PROPS = [];
                $Res = CIBlockElement::GetByID($this->arResult['UF_ITEM_ID']);
...
//                debmes(unserialize($this->arResult['UF_PROPERTIES']));
                ...
                if ($isAuthorized) {
                    $responseCheckList = new \Democontent2\Pi\CheckList\Response(intval($this->arResult['UF_ITEM_ID']));
                ...
                $response->setTaskId(intval($this->arResult['UF_ITEM_ID']));
                ...
                $reviews = new \Democontent2\Pi\Iblock\Reviews();
                $reviews->setTaskId(intval($this->arResult['UF_ITEM_ID']));
                ...
                if ($this->arResult['IS_OWNER']) {
                                ...
                                if ($request->getPost('setOption')) {
                                    switch ($request->getPost('setOption')) {
                                        case 'quickly':
                                            ...
                                            $order->setItemId($this->arResult['UF_ITEM_ID']);
                                            $order->setAdditionalParams(
                                                [
                                                    $this->arResult['UF_IBLOCK_ID'] => [$this->arResult['UF_ITEM_ID']]
                                                ]
                                            );
                                                    ...
                                                    if ($this->arResult['UF_SAFE'] && $safeCrowOrdersCount > 0) {
                                                        if (strlen(\Bitrix\Main\Config\Option::get(DSPI, 'safeCrowApiKey')) > 0
                                                            ...
                                                            foreach ($this->arResult['SAFECROW_ORDERS'] as $safeCrowOrder) {
                                                                if (($safeCrowOrder['UF_TASK_ID'] == $this->arResult['UF_ITEM_ID'])
                                                                    && (intval($safeCrowOrder['UF_STAGE_ID']) == intval($request->getPost('stageId')))) {
                                                                    $safeCrow = new \Democontent2\Pi\Payments\SafeCrow\SafeCrow();
                                                                    $safeCrow->setTaskId($this->arResult['UF_ITEM_ID']);
                                                                    ...
                                                                    $reviews->setTaskId($this->arResult['UF_ITEM_ID']);
                                    ...
                                    if ($executor > 0) {
                                        ...
                                        if ($this->arResult['UF_SAFE'] && $safeCrowOrdersCount > 0) {
                                            if (strlen(\Bitrix\Main\Config\Option::get(DSPI, 'safeCrowApiKey')) > 0
                                                && strlen(\Bitrix\Main\Config\Option::get(DSPI, 'safeCrowApiSecret')) > 0) {
                                                foreach ($this->arResult['SAFECROW_ORDERS'] as $safeCrowOrder) {
                                                    if ($safeCrowOrder['UF_TASK_ID'] == $this->arResult['UF_ITEM_ID'] && !intval($safeCrowOrder['UF_STAGE_ID'])) {
                                                        $safeCrow = new \Democontent2\Pi\Payments\SafeCrow\SafeCrow();
                                                        $safeCrow->setTaskId($this->arResult['UF_ITEM_ID']);
                                                        $closeSafeCrowOrder = $safeCrow->closeOrder(
                                                            $safeCrowOrder['UF_ORDER_ID'],
                    ...
                    if ($this->arResult['ALLOW_CHECKLISTS']) {
                        $responseCheckListValues = new ResponseValues(intval($this->arResult['UF_ITEM_ID']), 0);
                    ...
                    if (intval($consumer['UF_DSPI_SAFECROW_ID']) > 0 && intval($supplier['UF_DSPI_SAFECROW_ID']) > 0) {
                        ...
                        if ($orderId > 0) {
                            ...
                            if (count($executorCard) > 0) {
                                ...
                                $safeCrow->setExtra(
                                    [
                                        'taskId' => intval($this->arResult['UF_ITEM_ID']),
            ...
            if (count($orderId) == 2 && intval($orderId[0]) > 0) {
                ...
                if (isset($checkSafeCrowOrder['id']) && intval($checkSafeCrowOrder['id'])) {
                    switch ($checkSafeCrowOrder['status']) {
                        case 'paid':
                            if (isset($checkSafeCrowOrder['extra']['taskId']) && intval($checkSafeCrowOrder['extra']['taskId']) == intval($this->arResult['UF_ITEM_ID'])) {
    ...
    {
        if (isset($this->arResult['UF_ITEM_ID'])) {
            global $USER;
            $stat = new \Democontent2\Pi\Iblock\Stat(intval($this->arResult['UF_ITEM_ID']), intval($this->arResult['UF_IBLOCK_ID']));
